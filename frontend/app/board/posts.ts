...
export const POSTS: Post[] = [
  {
    id: 'privacy-policy-dev',
    title: '개인정보처리방침 개발 건에 관하여',
    category: 'PPT',
    date: '2026-07-16',
    summary: '급하게 떨어진 법령 문서 프론트엔드 요청을 처리한 기록 (슬라이드 6장)',
    tags: ['개인정보처리방침', '법령', 'frontend', '슬라이드', 'AI활용'],
    blocks: [
      {
        type: 'text',
    ...
    slides: [
      {
        ...
        blocks: [
          {
            type: 'text',
            content: `**개인정보처리방침 수정 작업에 관한 기록이다.**

상황은 이랬다. 갑자기 급하다며, 내가 속한 부서의 SR 요청도 아닌데 부탁을 받아 맡게 됐다.
회사 PC에서 SVN만 연결하면 바로 적용되는 건이라, 얼렁뚱땅 시작하게 됐다.

3일 안에 처리해달라고는 하는데 요구사항도 불명확해서 화가 났지만,
일단 받기로 했다. 급하다고 하니까`,
          },
        ],
      },
      {
        title: '요청서 확인 — 로직보다 CSS가 걸렸다',
        blocks: [
          {
            type: 'image',
            src: '/images/privacy-request.svg',
            caption: '요청 자체는 frontend 처리라 로직상 어려움이 없었다',
          },
          {
            type: 'text',
            content: `요청서를 보니 frontend 처리라 로직상 어려운 부분은 없었다.

다만 눈에 밟힌 건 CSS인데 id 기준으로 같은 스타일을 10개나 반복하는데 
당연히 클래스로 하지않나..? 싶은 부분이었다.
하지만 내 담당 시스템도 아니고 사이드 이펙트도 있으니 가만 뒀다.`,
          },
        ],
      },
      {
        title: '법령 조문 체계에 맞춰 표기 교체',
        blocks: [
          {
            type: 'image',
            src: '/images/privacy-jomun.svg',
            caption: "1. · 2. 같은 표기를 조 · 항 · 호 · 목 체계(제1조 / ① / 1. / 가.)로 맞췄다",
          },
          {
            type: 'text',
            content: `기존 처리방침과 요청서의 권장안을 비교하니 또 하나 눈에 밟히는 게 있었다. '1.', '2.' 표기였다.

법령 조문 체계에 따르면 조·항·호·목은 각각 '제1조 / ① / 1. / 가.' 형식으로 표기해야 한다.
(물론 몰랐던 규칙이지만 중요한 법적 문서라면 지키는 게 맞다고 보고 적용한 것)

결국 위의 그림처럼 표기를 맞췄는데 특수문자를 한 땀 한땀  찾는 게 너무 버거웠고
그렇게 탄생한 것이 '특수문자 모음.html'이다. 나중에 폐쇄망인 곳에서 또 써먹을 생각이다.`,
          },
        ],
      },
      {
        title: '플로우차트 3종 + 접수 부서 표 추가',
        blocks: [
          {
            type: 'image',
            src: '/images/privacy-flowcharts.svg',
            caption: '열람 요구 처리 · 인터넷 청구 · 서면 청구 절차도 3종과 접수 부서 표를 새로 넣었다',
          },
          {
            type: 'text',
            content: `그다음은 플로우차트 3종과 접수 부서 표를 추가했다.

CSS 파일을 건드리는 건 부담스러워서, style 속성으로 직접 처리했다.`,
          },
        ],
      },
      {
        ...
        blocks: [
          {
            type: 'image',
            src: '/images/privacy-grammar.svg',
            caption: '조사 누락, 표준 문안 압축, 낫표 추가, 능동·피동 정돈까지 4가지 유형을 교정했다',
          },
          {
            type: 'text',
            content: `이 요청은 내용을 더 다듬어야 한다고 생각해, 관련 법률과 문법을 law.go.kr에서 가져왔다.

기존 문장의 조사 누락을 바로잡고, 제1조는 표준 문안으로 압축했다.
「개인정보 보호법」처럼 법령명에 낫표를 일관되게 붙였고 능동·피동 표현도 정돈했다.`,
          },
        ],
      },
      {
        ...
        blocks: [
          {
            type: 'image',
            src: '/images/privacy-done.svg',
            caption: '로직 하나 없이도 꽤나 번거로웠던 작업이 깔끔하게 마무리됐다',
          },
          {
            type: 'text',
            content: `이렇게 로직 하나 없이도 꽤나 번거로운, 법령 관련 프론트엔드 요청을 마무리했다.

어떤 요청을 받더라도 그 요청의 needs, 방향성 등을 보는게 중요해졌다
`,
          },
        ],
      },
...
AI에게 제대로 된 질문을 던지려면 결국 스스로 질문을 던져야한다.`,
  ...
  },
  {
    ...
    title: 'Elasticsearch에 관하여',
    category: '개발',
    date: '2026-04-17',
    summary: '역색인, 형태소 분석, TF-IDF까지',
    tags: ['Elasticsearch', '검색엔진', '역색인', '백엔드'],
    blocks: [
      {
        type: 'text',
        content: `국가지원사업 관련해서 Elasticsearch에 관한 내용이 나왔다.
        자영업자 대상으로 온라인으로 쉽게 문서처리를 할 수 있게 한다는 것인데
        사업을 새롭게 제안하면서 접근성, 편의성을 높이기 위해 Elasticsearch을 쓴다는 내용이었다.
        RDBMS에서 자주 사용하는 Like 검색방식은 
        데이터 전체를 훑는 방식에 있어서 데이터가 많이 쌓일수록 효율도 떨어진다
        Elasticsearch의 경우 단어별로 색인을 만들어두고
        특정 단어가 검색이 되면 책갈피처럼 데이터중에 몇번째에 있었는지
        찾아주는 방식이므로 Full Scan 방식보다 효율적인 검색이 가능하다.
        `,
      },
      {
        type: 'demo',
        demoId: 'es-search-compare',
        label: 'LIKE 방식과 Elasticsearch 방식',
      },
      {
        type: 'text',
        content: `
        또한 RDBMS의 검색 방식은 단순 텍스트 검색이기 때문에 단어의 의미를 파악하지 못한다.
        Elasticsearch는 색인을 통해 단어가 구분하는데 이에 의문이 생겼다.
        어디까지가 하나의 단어인지를 판단할 수 있을까?
        언어마다 방식이 다를 수도 있고 신조어가 있을 수도 있는데?
        조사, 띄어쓰기 등으로 파악하기 어렵지않을까 하는 문제도 있다.
        `,
      },
      {
        type: 'demo',
        demoId: 'es-inverted-index',
        label: '문서가 색인되는 과정',
      },
      {
        type: 'text',
        content: `
        그래서 언어마다 각 언어에 맞는 분석기가 있고 (한국은 Nori 분석기)
        이를 사용하면 조사나 문장 구성에 따라 단어를 구분하여 색인으로 만들 수 있다.
        그 후 TF-IDF(Term Frequency-Inverse Document Frequency)를 이용한다.
        문서 내 단어 빈도와 전체 문서 집합 내 단어 희소성을 곱하여 
        특정 문서에서 해당 단어가 얼마나 중요한지 측정하는 가중치 지표인데
        신조어의 경우, 초반엔 DB 내 빈도가 낮아 점수가 낮지만
        사용량이 늘어나면 점수가 올라가고 유의미한 단어로 인식되기 시작한다.`,
      },
      {
        type: 'text',
        content: `다만 Nori 분석기는 "사전 + 형태소 규칙"으로 자르기 때문에
        사전에 없는 단어는 형태소 경계를 잘못 판단할 수 있어서
        이를 보완하기 위해 n-gram 도 병행한다.
        n-gram은 글자를 일정 단위로 기계적으로 쪼개는 방식인데
        예를 들어 "남남수수학학원원장"의 경우,
        nori에 의해 "수학","학원", "원장" 등은 잡히겠지만
        "남남수"는 없는 단어이니 n-gram에서 
        기계적으로 처리하여 "남남","남수" 등으로 쪼개질 수 있다.
        하지만 이 경우, "남남수수학학원원장" 자체가 밈으로 쓰이니
        원문으로 검색했을때가 가장 유효하다고 볼 수 있다.`,
      },
      {
        type: 'text',
        content: `이에 대한 생각을 하면서 자동완성에 관한 경험이 떠올랐는데
        검색어를 입력하다보면 분명 단어는 수도 없이 많은데 
        묘하게 내 관심사쪽으로 유도하고 있다는 느낌을 받았다.
        자동완성의 경우, Elasticsearch가 후보를 뽑는 것까지는 비슷하지만
        내 검색이력과 패턴을 기반으로 순서가 바뀐다고 한다.
        Elasticsearch가 "무엇이 있는지"를,
        개인화 로직이 "개인에 맞는 정보"를 재정렬하는 것으로
        자동완성의 기능을 향상시켜주고 있다.`,
      },
      // {
      //   type: 'demo',
      //   demoId: 'es-autocomplete',
      //   label: '자동완성 n-gram 시뮬레이션',
      // },
    ],
  },
  {
    ...
    blocks: [
      {
        type: 'text',
        content: `
        중학교 시절 한창 아프리카에서 게임방송이 떠오르던 때가 있었다. 
        풍월량, 우왁굳, 대정령 같은 1세대 방송인들이 떠오르고 
        온게임넷에서 스타리그, 롤 대회 송출해주던 시절
        지금 생각해보면 당시 인기가 많던 아프리카TV는 문제가 많았다.
        
        방송을 보려면 ActiveX가 필수여서 사용자들이 불편하다 아우성이었다.
        또 버릴 수는 없는게 서버비용 때문에 P2P(Peer to Peer)를 사용했는데
        시청자의 PC를 서버로 취하기 위해 ActiveX를 받아야하는 불편함뿐만 아니라
        이는 PC의 권한을 가져오는 것이므로 취약점 문제도 많았으며
        당시 브라우저중에서도 상당히 양아치로 알려진 IE를 사용해야했던 것도 한 몫했다.
        (IE의 경우 웹 표준 규약 안지키다가 edge로 바뀌고 지키기 시작했다.)
        P2P는 아프리카TV의 서버가 영상을 전부에게 전달하는게 아니라
        시청자들끼리 영상을 나눠서 전달하는, 시청자도 서버가 되는 방식이므로
        시청자의 인터넷 상태가 안좋으면 그 이후 접속자의 연결 또한 불안정해지는 등의 문제도 있었다.
        
        이런 문제들은 결국 두 가지 해결 방안이 있었는데
        하나는 CDN(Content Delivery Network)의 보편화다.
        P2P와 반대로 서버가 직접 모든 시청자에게 영상을 뿌리는 방식이다.
        비용이 훨씬 많이 들지만 품질은 확실히 보장되며
        트위치와 유튜브 라이브가 이 방식으로 치고 들어왔고
        아프리카TV는 결국 경쟁을 위해 따라갈 수밖에 없었다.
 
        다른 하나는 웹 표준화다. 
        HTML5 Video가 자리잡으면서 플러그인 없이도 영상 재생이 가능해졌다. 
        ActiveX는 IE와 함께 사라져갔고, HLS(HTTP Live Streaming)라는 프로토콜이 스트리밍 표준으로 자리잡았다. 
        이게 지금 우리가 크롬에서 아무 설치 없이 방송을 볼 수 있는 이유다.
        
        다른 기술들에 비해 뒤쳐졌다고 여겨진 P2P는 WebRTC라는 이름으로 브라우저 표준에 녹아들었다.
        플러그인 없이, 브라우저끼리 직접 연결해 실시간으로 데이터를 주고받는 기술이다. 
        화상통화, 화상회의가 별도 앱 없이 브라우저에서 되는 게 다 WebRTC 덕분이다. 
        욕을 먹던 그 아이디어가 표준이 되어 돌아온 셈이다. 
        기술은 사라지는 게 아니라 더 나은 형태로 다시 나타난다는 걸 보여주는 케이스다.

        아프리카TV는 2024년 SOOP으로 리브랜딩했다. 때마침 네이버가 치지직을 들고 나왔고, 숲과 치지직의 경쟁 구도가 됐다.
        스트리밍 시장은 결국 플랫폼 싸움이 됐다. 기술 문제는 해결됐고, 이제 남은 건 BJ와 시청자를 누가 더 많이 잡느냐다.
        그리고 그건 개발자가 아니라 비즈니스가 풀어야 할 문제다.
