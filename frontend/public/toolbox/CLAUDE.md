...
4. **도구 파일 안에 주석을 남기지 않는다** — 변경 이력·설계 메모는 `개선사항_메모.md`(git 제외)에 날짜와 함께 적는다. 주석이 들어갔으면 커밋 전에 `dev_tools.html` 화면 표시기의 주석 삭제(XML·HTML 규칙)로 걷어낸다. 2026-09-12 에 이력째 걷어냈다.
...
`frontend/app/toolbox/page.tsx` 가 `toolbox.html` 의 런처 배열을 빌드 때 읽어 `/toolbox` 화면 카드를 만든다 — 객체 키(`file`·`icon`·`name`·`desc`)를 바꾸면 그쪽 정규식도 같이 고친다.
...
| `dev_tools.html` 화면 표시기의 주석 삭제 규칙(언어 추가·주석 기호 변경) | **`doc/주석문법_언어별.md`** — 언어별 주석·문자열·통과구간이 **문서 인용 / 실측 / 미확인** 3등급으로 갈려 있다. SQL 힌트 `--+` 처럼 지우면 실행계획이 바뀌는 항목이 여기 있다 |
...
`node scripts/toolbox-lint.js <파일>` 이 「절대 규칙」 1(외부 CDN·npm 금지)과 4(주석 금지)를 센다. 파일을 고치면 hook 이 `doc-lint.sh` 를 거쳐 이것을 부르므로 따로 돌릴 일은 드물다. 주석 기호를 데이터로 들고 있는 자리(주석 제거기의 델리미터 표, SQL 힌트 문자열, 정규식)는 문자열·정규식을 건너뛰며 훑어서 안 센다.
...
도구 안의 라벨·설명·안내 문구는 루트 [CLAUDE.md](../../../CLAUDE.md)의 글 작성 규칙을 따른다.

거기에 더해 여기서만 거는 것이 하나 있다 — 설명란 첫 문장에 `무엇을` `무엇으로` `어떻게`가 다 들어가야 한다.
처음 여는 사람이 도구 이름만 보고는 무엇을 넣을지 모르는 자리라서다. 루트 규칙에서는 뺐다.
